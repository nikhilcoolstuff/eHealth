//
//  SFSlideMenuRootViewController.m
//  SFSlideMenu
//
...
typedef enum {
    SFSlideMenuStateInitial,
    SFSlideMenuStateContent,
    SFSlideMenuStateMenu,
    SFSlideMenuStateRightMenu
} SFSlideMenuState;

typedef enum {
    SFSlideMenuPanningStateStopped,
    SFSlideMenuPanningStateRight,
    SFSlideMenuPanningStateLeft
} SFSlideMenuPanningState;

@interface SFSlideMenuRootViewController (){
    SFSlideMenuState state;
    SFSlideMenuPanningState panningState;
...
@implementation SFSlideMenuRootViewController

@synthesize menuView;

#pragma mark -
#pragma mark SFSlideMenuRootViewController
...
-(void) doSlideIn:(void (^)(BOOL completed))completion{
    ...
    [UIView animateWithDuration:duration delay:0.0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        ...
        if (panningState == SFSlideMenuPanningStateLeft
            || state == SFSlideMenuStateRightMenu) {
    ...
    if ([gesture state] == UIGestureRecognizerStateBegan) {
        ...
        if (movingView.frame.origin.x + translation.x < 0 ) {
            if (self.isRightMenuEnabled && self.rightMenu != nil) {
                panningState = SFSlideMenuPanningStateLeft;
                [self addRightMenu];
            }else{
                translation.x = 0.0;
                panningState = SFSlideMenuPanningStateRight;
            }
        }else{
            panningState = SFSlideMenuPanningStateRight;
        }
        ...
        if (panningState == SFSlideMenuPanningStateRight) {
...
-(void) switchToContentViewController:(UINavigationController*) content completion:(void (^)(void))completion{
    ...
    if (state != SFSlideMenuStateInitial || hideContentOnStartup) {
        if (slideOutThenIn) {
        ...
        [self slideIn:self.selectedContent];
        if (state == SFSlideMenuStateRightMenu) {
            [self.rightMenu willMoveToParentViewController:nil];
            [self.rightMenu.view removeFromSuperview];
            [self.rightMenu removeFromParentViewController];
        }
...
-(void) pushRightNavigationController:(SFSlideMenuNavigationController*)navigationController{
    SFSlideMenuRootViewController* root = self;
...
-(void) viewDidLoad{
    ...
    state = SFSlideMenuStateInitial;
    panningState = SFSlideMenuPanningStateStopped;
